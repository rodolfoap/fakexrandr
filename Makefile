...
install: libXrandr.so
	...
	[ -d $$TARGET_DIR ] || exit 1; \
	sudo install libXrandr.so $$TARGET_DIR; \
	sudo ln -s libXrandr.so $$TARGET_DIR/libXrandr.so.2 || true; \
	sudo ln -s libXrandr.so $$TARGET_DIR/libXinerama.so.1 || true; \
	sudo ldconfig
	sudo install fakexrandr-manage.py $(PREFIX)/bin/fakexrandr-manage

uninstall: config.h
	TARGET_DIR=`sed -nre 's/#define FAKEXRANDR_INSTALL_DIR "([^"]+)"/\1/p' config.h`; \
	[ -d $$TARGET_DIR ] || exit 1; \
	strings $$TARGET_DIR/libXrandr.so | grep -q _is_fake_xrandr || exit 1; \
	sudo rm -f $$TARGET_DIR/libXrandr.so $$TARGET_DIR/libXrandr.so.2 $$TARGET_DIR/libXinerama.so.1 $(PREFIX)/bin/fakexrandr-manage; \
	ldconfig
